...
import { ArrowLeft, CheckCircle2, XCircle, Circle, RotateCcw, Trophy, BookOpen, Cpu, Globe, Binary, Search, Shuffle, ChevronRight } from 'lucide-react';
...
// ─── Subject Config ─────────────────────────────────────────────────────────────
// Actual domain layout in the HuggingFace dataset:
//   DSA               rows 0–399    (4 fetches)
//   Computer Org      rows 400–999  (6 fetches)
//   Computer Network  rows 1000–1099(1 fetch, ~74 CN rows + some OS)
//   Operating System  rows 1100–1336(3 fetches)
const SUBJECTS = [
  {
    id: 'dsa',
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-2">
          ...
          {filtered.map((q, idx) => (
            <button
              ...
              className="group w-full text-left flex items-center gap-4 bg-slate-900/60 hover:bg-slate-800/60 border border-slate-800 hover:border-slate-700 rounded-xl px-4 py-3.5 transition-all"
            >
              {/* Number */}
              <span className="text-xs font-bold text-slate-600 w-8 shrink-0 text-right">
              ...
              <div className="flex-1 min-w-0">
                ...
                {q.subDomain && (
                  <span className="inline-block text-xs mt-1 text-slate-500">{q.subDomain}</span>
                )}
              </div>
...
function Quiz({ subject, questions, onFinish, onBack }) {
  ...
  const handleNext = () => {
    if (currentIdx < questions.length - 1) {
      ...
      onFinish(score + (selected === question.correct ? 0 : 0), score + (selected === question.correct ? 1 : 0));
    }
  };
        ...
        <div className="max-w-2xl w-full">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-8">
            <p className="text-xs font-medium text-slate-500 mb-3 uppercase tracking-wider">
              ...
              {question.subDomain && <span className="ml-2 normal-case">· {question.subDomain}</span>}
            </p>
            <h2 className="text-xl font-medium mb-7 leading-relaxed text-slate-100">{question.question}</h2>
              ...
              {question.options.map((opt, idx) => {
                ...
                if (isAnswered) {
                  if (idx === question.correct) {
                    style = 'border-green-500/60 bg-green-500/10 text-green-100';
                  } else if (idx === selected) {
                    style = 'border-red-500/60 bg-red-500/10 text-red-100';
                  } else {
                ...
                return (
                  <button
                    ...
                    disabled={isAnswered}
                    className={`text-left p-4 rounded-xl border flex items-center gap-3 transition-all duration-200 ${style} ${!isAnswered ? 'cursor-pointer' : 'cursor-default'}`}
                  >
                    <span className={`text-xs font-bold w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${
                      isAnswered && idx === question.correct ? 'bg-green-500/30 text-green-300' :
                      isAnswered && idx === selected ? 'bg-red-500/30 text-red-300' :
              ...
              <div className="mt-7 pt-6 border-t border-slate-800">
                {question.explanation && (
                  <p className="text-slate-300 text-sm mb-5 leading-relaxed">
                    <strong className="text-slate-100 font-medium">Explanation: </strong>
                    {question.explanation}
                  </p>
                )}
                <div className="flex justify-between items-center">
                  <span className={`text-sm font-medium ${selected === question.correct ? 'text-green-400' : 'text-red-400'}`}>
...
function Results({ subject, score, total, onRetry, onBrowse, onChangeSubject }) {
  ...
  const gradeColor = pct >= 80 ? 'text-green-400' : pct >= 60 ? 'text-blue-400' : pct >= 40 ? 'text-yellow-400' : 'text-red-400';
    ...
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full bg-slate-900/80 border border-slate-800 rounded-2xl p-10 text-center">
        ...
        <div className="h-2 bg-slate-800 rounded-full mb-8 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-700 ${pct >= 80 ? 'bg-green-500' : pct >= 60 ? 'bg-blue-500' : pct >= 40 ? 'bg-yellow-500' : 'bg-red-500'}`}
            style={{ width: `${pct}%` }}
          />
        </div>
...
export default function MCQ() {
  ...
  const [allQuestions, setAllQuestions] = useState([]); // all fetched questions for subject
  const [quizQuestions, setQuizQuestions] = useState([]); // 10-question quiz set
  const [score, setScore] = useState(0);
  const [error, setError] = useState(null);

  // Fetch ALL questions for the selected subject using subject-specific offsets
  ...
  // Start quiz from a specific question (that question first, then 9 random others)
