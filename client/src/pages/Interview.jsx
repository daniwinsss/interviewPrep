    ...
    <div className="h-screen page flex flex-col overflow-hidden">
      <header className="h-16 border-b border-white/10 bg-[#0f0f10]/80 backdrop-blur-xl flex items-center px-6 gap-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-semibold text-white">AI Interviewer</h1>
            <p className="text-xs text-white/60">Live coding interview</p>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {isDsa && (
            <div className="relative">
              <select
                ...
                disabled={isStarting}
                className="appearance-none bg-white/5 border border-white/10 text-white text-sm px-3 py-2 pr-8 rounded-xl focus:outline-none focus:border-white/30 cursor-pointer"
              >
          ...
          <select
            ...
            className="appearance-none bg-white/5 border border-white/10 text-white text-sm px-3 py-2 rounded-xl focus:outline-none focus:border-white/30 cursor-pointer"
          >
            {TOPICS.map((item) => (
      ...
      {isProject && (
        <div className="px-6 pt-4">
          <div className="max-w-7xl mx-auto glass p-5 flex flex-col gap-3">
            <label className="text-xs uppercase tracking-[0.3em] text-white/50">GitHub Repository</label>
            <input
              type="url"
              value={repoUrl}
            ...
            <button
              ...
              disabled={isStarting || isTyping}
              className="btn-secondary text-sm self-start disabled:opacity-50"
            >
      ...
      {error && (
        <div className="px-6 pt-4">
          <div className="max-w-7xl mx-auto bg-white/5 border border-white/20 text-white text-sm px-4 py-3 rounded-2xl">
            {error}
          </div>
        </div>
      )}
          ...
          <div className="px-6 py-4 border-b border-white/10 bg-white/5 shrink-0">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                {repoName && (
                  <p className="text-sm text-white/70 mt-1">{repoName}</p>
                )}
                {currentPhase && (
                  <p className="text-xs text-white/50 uppercase tracking-[0.3em] mt-1">
            ...
            {isDsa && problemContent && (
              <p className="text-sm text-white/60 mt-3 line-clamp-3">{problemContent}</p>
            )}
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
            {isStarting ? (
              <div className="flex justify-start">
                <div className="bg-white/5 border border-white/10 rounded-2xl rounded-tl-sm p-4 flex items-center gap-2 text-white/60">
              ...
              messages.map((msg, idx) => (
                <div key={msg._id || idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[88%] rounded-2xl p-4 ${
                    msg.role === 'user'
                      ? 'bg-white text-black border border-white/60 rounded-tr-sm'
            ...
            {isMonitoringCode && (
              <div className="flex justify-start">
                <div className="bg-white/10 border border-white/20 rounded-2xl rounded-tl-sm p-4 flex items-center gap-2 text-white text-sm">
                  <Loader2 className="w-4 h-4 animate-spin" />
          ...
          <div className="p-4 bg-[#0f0f10]/90 border-t border-white/10 backdrop-blur-md shrink-0">
            <div className="max-w-full flex gap-3">
              ...
              <button
                ...
                className="bg-white text-black disabled:opacity-50 disabled:hover:bg-white p-3 px-6 rounded-2xl font-semibold transition-all flex items-center gap-2"
              >
                <span>Send</span>
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-[#0f0f10]">
            <div className="h-12 border-b border-white/10 flex items-center justify-between px-4 bg-white/5 shrink-0">
              ...
              <button
                ...
                className="text-xs px-3 py-1.5 rounded-xl border border-white/20 bg-white/10 text-white hover:bg-white/20 disabled:opacity-50"
              >
